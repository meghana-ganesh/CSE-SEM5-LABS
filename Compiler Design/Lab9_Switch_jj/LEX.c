...
const char *Keywords[] = {"const", "char", "int", "return", "for", "while", "do", "switch", "if", "else", "unsigned", "case", "break","default"};
...
struct token getNextToken(FILE *fin){
    ...
    while (!gotToken && (c = fgetc(fin)) != EOF){
        if (charBelongsTo(c, specialsymbols)){
            ...
            tkn.row = row;
            strcpy(tkn.type,"CharacterLiteral");
            tkn.lexeme[0]= '\'';
            c = fgetc(fin);
            ++col;   
            tkn.lexeme[1] = c;
